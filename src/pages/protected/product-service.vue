  ...
  return [
    ...
    },
    {
      ...
      onSelect() {
        ...
        toast.add({ title: "Copied Product/Service ID", color: "success" });
      },
    },
    {
      type: "separator",
      ...
      async onSelect() {
        modal.open({
          ...
          onSubmit: async (formdata) => {
            const error = await productService.updateProdcutService(row.original.id, formdata);
            if (!error) {
              toast.add({ title: "Product/Service has been updated", color: "success" });
            } else {
              toast.add({ title: "Product/Service dailed to update", color: "warning" });
            }
    ...
    },
    {
      ...
      icon: "i-lucide-trash",
      async onSelect() {
        const error = await productService.deleteProductService(row.original.id);
        if (!error) {
          toast.add({ title: "Product/Service has been deleted", color: "success" });
        } else {
          toast.add({ title: "Product/Service dailed to delete", color: "warning" });
        }
      },
    },
  ];
...
async function onGetCsv() {
  const error = await productService.getCSV();
  if (!error) {
    toast.add({ title: "Product/Service Exported", color: "success" });
  } else {
    toast.add({ title: "Product/Service failed to export", color: "warning" });
  }
}
  ...
  modal.open({
    ...
    onSubmit: async (formdata) => {
      ...
      if (!error) {
        ...
        toast.add({ title: "Product/Service Failed to create", color: "warning" });
      }
  ...
  <dashboardWrapper title="Product/Service">
    <genericTool :loading="loading" v-model:search="search" @search="Onsearch" @csv-get="onGetCsv">
      <UButton @click="openModal" icon="i-lucide-plus" class="shrink-0"
        >Create Product/Service</UButton
      >
    </genericTool>
    <genericTable
      v-if="productsServices?.record"
      v-model:page="page"
      :data="productsServices?.record"
      :total-record="productsServices?.meta.totalRecord"
      :limit="productsServices?.meta.limit"
      :columns="columns"
    />
  </dashboardWrapper>
</template>
